...
async function create(payload) {
  ...
  const [result] = await pool.query(
    `INSERT INTO planes_libertad (usuario_id, meta, fecha_inicio, fecha_fin, estado)
     VALUES (?, ?, ?, ?, ?)`,
  ...
  return {
    ok: true,
    data: newItem,
    message: "Plan de libertad creado correctamente",
  };
}
...
async function update(id, payload) {
  ...
  if (fields.length > 0) {
    values.push(id);
    await pool.query(`UPDATE planes_libertad SET ${fields.join(", ")} WHERE id = ?`, values);
  }
  ...
  return {
    ok: true,
    data: updatedItem,
    message: "Plan de libertad actualizado correctamente",
  };
}

async function remove(id) {
  await pool.query("DELETE FROM planes_libertad WHERE id = ?", [id]);
  return true;
}
